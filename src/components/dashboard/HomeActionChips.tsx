  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-3">
        <button type="button" onClick={() => setOpenView('appts')} className="flex items-center gap-3 rounded-[24px] bg-white px-4 py-3.5 text-left shadow-sm ring-1 ring-zinc-200/60 transition active:scale-[0.98] active:bg-zinc-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2">
          ...
          <span className="min-w-0">
            <span className="block text-2xl font-bold leading-none tabular-nums text-zinc-900">{appts.length}</span>
            <span className="mt-0.5 block text-xs text-zinc-500">Ραντεβού</span>
          </span>
        </button>
        <button type="button" onClick={() => setOpenView('callbacks')} className="flex items-center gap-3 rounded-[24px] bg-white px-4 py-3.5 text-left shadow-sm ring-1 ring-zinc-200/60 transition active:scale-[0.98] active:bg-zinc-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2">
          ...
          <span className="min-w-0">
            <span className="block text-2xl font-bold leading-none tabular-nums text-zinc-900">{callbacks.length}</span>
            <span className="mt-0.5 block text-xs text-zinc-500">Να πάρω τηλέφωνο</span>
          </span>
        </button>
      </div>
      ...
      {openView && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end" role="dialog" aria-modal="true">
          ...
          <div className="relative mx-auto max-h-[80dvh] w-full max-w-2xl overflow-y-auto rounded-t-[28px] bg-white px-4 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-3 shadow-2xl motion-safe:animate-[sheetUp_0.22s_ease-out]">
            ...
            {list.length === 0 ? (
              ...
              <div className="space-y-1.5 pb-2">
                {list.map((it) => (
                  <div key={it.id} className="flex items-center gap-2 rounded-[24px] bg-zinc-50 px-3 py-2.5 ring-1 ring-zinc-200/60">
                    ...
                    {it.phone && (
                      <a href={`tel:${it.phone}`} aria-label="Κλήση" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white text-indigo-600 ring-1 ring-zinc-200/60 transition active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2">
                        <svg className="h-5 w-5" fill="none" strokeWidth={1.7} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z" /></svg>
                      </a>
                    )}
                  </div>

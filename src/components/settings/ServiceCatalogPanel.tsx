  ...
  return (
    <div className="mt-4 rounded-[28px] bg-white px-5 py-4 shadow-sm ring-1 ring-zinc-200/60">
      ...
      <div className="mt-3 space-y-2 rounded-2xl bg-zinc-50 p-3 ring-1 ring-zinc-200/60">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Όνομα υπηρεσίας/υλικού" className="w-full rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-base outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500" />
        <div className="flex gap-2">
          <input value={code} onChange={(e) => setCode(e.target.value)} placeholder="Κωδ." className="w-20 rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-base outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500" />
          <input value={unit} onChange={(e) => setUnit(e.target.value)} placeholder="Μον. (τεμ./ώρα)" className="flex-1 rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-base outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500" />
          <input value={price} onChange={(e) => setPrice(e.target.value)} inputMode="decimal" placeholder="€" className="w-20 rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-base tabular-nums outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500" />
          <input value={vat} onChange={(e) => setVat(e.target.value)} inputMode="decimal" placeholder="ΦΠΑ%" className="w-16 rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-base tabular-nums outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500" />
        </div>
      ...
      <div className="mt-3 space-y-1.5">
        {loading ? (
          ...
          items.map((it) => (
            <div key={it.id} className="flex min-h-[44px] items-center gap-2 rounded-xl px-2 py-2.5 hover:bg-zinc-50">
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-zinc-900">
                  {it.code ? <span className="text-zinc-400">{it.code} · </span> : null}{it.name}
                </p>
                <p className="text-xs text-zinc-500"><span className="tabular-nums">€{it.unitPrice.toLocaleString('el-GR')}</span>{it.unit ? ` / ${it.unit}` : ''} · ΦΠΑ <span className="tabular-nums">{it.vatRate}</span>%</p>
              </div>
              <button type="button" onClick={() => removeItem(it.id)} aria-label="Διαγραφή" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-zinc-400 transition hover:bg-red-50 hover:text-red-500 active:bg-red-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2">

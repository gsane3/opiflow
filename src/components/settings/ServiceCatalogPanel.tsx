          ...
          items.map((it) => (
            <div key={it.id} className="flex min-h-[44px] items-center gap-2 rounded-xl px-2 py-2.5 hover:bg-zinc-50 dark:hover:bg-white/5">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-400"><span className="tabular-nums">{fmtEur(it.unitPrice)}</span>{it.unit ? ` / ${it.unit}` : ''} · ΦΠΑ <span className="tabular-nums">{it.vatRate}</span>%</p>
              </div>

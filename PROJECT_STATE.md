...
- **2026-06-09 — session 12 — 🎨 UX/UI REDESIGN (phone-first messenger) — plan + P0/P1 + P2a/P2b:**
  ...
  - **⏭️ NEXT:** **P4** service catalog (`service_catalog_items` table already migrated) → offer composer + home calendar/callback popups (`tasks.start_at/end_at` ready) · **P2c** Settings 6 categories + Κλήσεις customer-search bar + **SendChannelSheet unify + WhatsApp removal → re-apply migration 042** · **P5** AI assistant (voice / Siri-like popup, customer-scoped) · **P6** native engine (inbound + server recording + spoken consent + auto missed-call waterfall). The old `customers/[id]` stacked-card page still exists (now reached only by direct URL) — retire in a later pass.

...
| **Native app (NEW)** | **`E:\yorgos\native\`** — Expo SDK 54, RN 0.81.5, New Arch ON, expo-router, brand #146EB4, bundle `ai.opiflow.app`. Reuses Supabase + Vercel `/api/*` (CORS via `src/proxy.ts`). |
...
| Twilio RN SDK | `@twilio/voice-react-native-sdk@2.0.0-preview.2` — **lazy-loaded** via `import('@/lib/twilio')` only (else release crashes at launch). |

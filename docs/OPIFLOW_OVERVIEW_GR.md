...
- **Firebase Cloud Messaging / APNs** → push στις native apps (αναπάντητες κλήσεις, εβδομαδιαία
  σύνοψη κ.λπ.). *Σημείωση: το non-call push δουλεύει σε **Android**· το iOS θέλει ακόμη ρύθμιση
  Firebase-iOS.*
...
7. **iOS killed-app calls** δεν χτυπάνε ακόμη (λείπει το APNs VoIP push credential στο Twilio).
